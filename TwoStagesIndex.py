...
def init_nn(stage):
    if stage==1:
        model = Sequential()
        model.add(Dense(units=2, input_shape=(1,), activation='relu', use_bias=True, dtype='float32'))
...
class RecursiveModelIndexes:
    ...
    def __init__(self, X, y, stages, threshold):
        ...
        self.models.append([])
        self.max_errs = [0 for i in range(stages[-1])]
    ...
    def train(self):
        ...
        y = self.y
        num = self.stages[1]
        tmp_records = [[] for i in range(num)]
        tmp_labels = [[] for i in range(num)]
        # stage1
        self.models[0][-1].fit(X, y, batch_size=256, epochs=3)
        ...
        for i in range(self.stages[1]):
            ...
            # TODO:若max_err超过threshold,则将该模型换位b树
        return 0
    ...
    def index(self, key):
        ...
        # TODO:在数组上查找x,用二分查找，第一个分裂点取pos,（应该将数组引用传进来）
        return pos
